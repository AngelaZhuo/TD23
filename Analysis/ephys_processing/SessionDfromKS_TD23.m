...
rewtrace = dchannels(:,rewchan(1))+dchannels(:,rewchan(2));  %TD23
% rewtrace = dchannels(:, rewchan); %TD19
...
if numel(rew)/2>numel(fv_on)/4
%     error('extra reward(s) during session!')
    error('extra drop(s)/fakedrop(s) during session!')
    %     d=nan;
    %     return
% elseif numel(rew)<numel(fv_on)/4
elseif numel(rew)/2<numel(fv_on)/4
%     error('reward(s) missing!')
    error('drop(s)/fakedrop(s) missing!')
    %     d=nan;
    %     return
end
...
%         events(i).reward_time = events(i).fv_off_rewcue+events(i).reward_delay/1000;
       ...
       error('No drop/fakedrop time for this trial!')
    end
