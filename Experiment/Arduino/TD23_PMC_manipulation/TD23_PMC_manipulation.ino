...
//          }
      ...
      //PMC paradigm with silencing at specific time point
       ...
       // == 0 means strings are equal
            ...
            reward_delay = (int)atoi(argv[4]);
            reward_size = (int)atoi(argv[5]);
            silence_period = (int)atoi(argv[6]); //silence either at CS1, CS2, CS1 delay or CS2 delay
            inhibit_or_not = (int)atoi(argv[7]);
            //odorcue_odor_dur = (int)atoi(argv[6]);
            //rewardcue_odor_dur = (int)atoi(argv[7]);
            lick_window = 7500;
...
// open FV for CS1
    ...
    case 3: // perceived odorcue_OdorOnTime + Optogenetic inhibition/sham start
        currentTime = millis();
        if  (odorON &&(currentTime - odorcue_OdorOnTime > odor_lat_On) && (silence_period == 1) && (inhibit_or_not == 1)) {        
        ...
        else if (odorON &&(currentTime - odorcue_OdorOnTime > odor_lat_On) && (silence_period == 1) && (inhibit_or_not == 0)) {
          perc_odorcue_OdorOnTime = millis();
...
// open FV for rewardcue odor
    ...
    case 10: // perceived rewardcue_OdorOffTime + Optogenetic inhibition/sham end or start
     ...
     if (drop_or_not == 0 && (currentTime - perc_rewardcue_OdorOffTime > reward_delay - drop_lat) && (silence_period == 4) && (inhibit_or_not == 0))  
     {
        DropOnTimefake = millis();
        Serial.println ("Dropfake");
        Serial.println (DropOnTimefake);
     ...
     state = 12; 
      ...
      if (drop_or_not == 1 && (currentTime - perc_rewardcue_OdorOffTime > reward_delay - drop_lat) && (strcmp(argv[6], "CS2_delay") == 0) && (inhibit_or_not == 1)) {       
      Serial.println ("Drop");
      digitalWrite(OptBlueTrig, LOW);
      OpenFaucet();                
      state = 12;
      ...
      if (drop_or_not == 1 && (currentTime - perc_rewardcue_OdorOffTime > reward_delay - drop_lat) && (strcmp(argv[6], "CS2_delay") == 0) && (inhibit_or_not == 0)) {       
      Serial.println ("Drop");
      digitalWrite(ShamBlueTrig, LOW);
      OpenFaucet();                
      state = 12;
