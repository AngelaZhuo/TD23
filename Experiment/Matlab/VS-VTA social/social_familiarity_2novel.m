function social_familiarity_2novel(COMPort, fname)
%% 
% compatible with flex_arduino_2023b
%
% Trialstructure:
% 90 trials - 30 per odor: familiar juvenile and 2 novel juveniles
...
load('D:\Experimental Control\VS-VTA social\CaseList.mat','CaseList');            
% Block 1: define trial parameters
nt = 30;            %trials per odor
case_num = [323, 324, 325];     %case number from ephys setup
...
trials2 = repmat([106,107],1,20)';
% random shuffle
trials2 = trials2(randperm(length(trials2)));

% pseudorandomness
while CheckRepetitions(trials2,3) > 0
   trials2 = trials2(randperm(length(trials2)));
end

trials = cat(1,trials, trials2);
...
for tr = 1:size(session.trialmatrix,2)
    ...
    switch session.trialmatrix(tr).case_num
        case 106
            s1 = sprintf('%s \r', 'laser_pattern_2');
        case 107
            s1 = sprintf('%s \r', 'laser_pattern_3');
        otherwise
