function [chapter, trialmatrix, ex_vectors_cur] = do_constructTrialMatrix_TD23_excitation(phase, AorB)%,JitterButton)
...
%         ex_vectors_cur=experiments(:,:,ChosenVec);
       % choose vectors randomly
        ...
        if AorB == 1
            ex_vectors_cur((ex_vectors_cur(:,1) == 0),5) = 0;
        elseif AorB == 2
            ex_vectors_cur((ex_vectors_cur(:,1) == 1),5) = 0;
        end
...
%        end
       ...
       [trialmatrix] = ConstructTrialMatrix_TD23_excitation(chapter,ex_vectors_cur);
